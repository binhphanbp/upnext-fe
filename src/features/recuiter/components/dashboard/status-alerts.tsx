...
export function StatusAlerts() {
  return (
    <div className="rounded-2xl border border-gray-150 bg-white p-5 shadow-2xs hover:shadow-xs transition-all duration-200 flex flex-col">
      <div>
        ...
        <div className="space-y-2">
          {alertItems.map((item, idx) => (
            <div
              key={idx}
              className="flex justify-between items-center text-[11px] border-b border-gray-50 pb-2"
            >
              <span
              ...
              <span
                className={`flex h-5 min-w-5 items-center justify-center rounded-full text-[9px] font-black px-1.5 ${item.color}`}
              >
                {item.count}
              </span>
            </div>
      ...
      <div className="mt-4 flex items-center justify-center gap-1.5 rounded-xl bg-emerald-50/75 px-3 py-2 text-[10px] font-extrabold text-emerald-700 shadow-3xs border border-emerald-100/40">
