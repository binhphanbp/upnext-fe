...
export function Input({ className, id, label, name, ...props }: InputProps) {
  ...
  return (
    <input
      {...props}
      className={cn(
        "h-11 w-full rounded-lg border border-border bg-white px-3 text-sm text-foreground outline-none transition focus:border-primary focus:ring-4 focus:ring-teal-100",
        className,
      )}
      id={inputId}
      name={name}
